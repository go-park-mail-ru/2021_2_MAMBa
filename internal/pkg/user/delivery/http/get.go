...
func (handler *UserHandler) GetBasicInfo(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		resp := domain.Response{Error: cast.StringToJson(customErrors.ErrIdMsg), Status: http.StatusNotFound}
		resp.Write(w)
		return
	}
...
func (handler *UserHandler) GetProfile(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil && err != customErrors.ErrorUserNotLoggedIn {
		resp := domain.Response{Error: cast.StringToJson(customErrors.ErrorInternalServer.Error()), Status: http.StatusInternalServerError}
		resp.Write(w)
		return
	}
	us, err := handler.UserUsecase.GetProfileById(clientID, targetID)
	if err != nil {
		resp := domain.Response{Error: cast.StringToJson(customErrors.ErrorBadInput.Error()), Status: http.StatusNotFound}
		resp.Write(w)
		return
	}
